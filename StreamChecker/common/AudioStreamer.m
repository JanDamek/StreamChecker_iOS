...
{	
	...
	AudioStreamer* streamer = (AudioStreamer*)CFBridgingRelease(inClientData);
	[streamer
...
void MyAudioQueueIsRunningCallback(void *inUserData, AudioQueueRef inAQ, AudioQueuePropertyID inID)
{
	AudioStreamer* streamer = (AudioStreamer *)CFBridgingRelease(inUserData);
...
{
	AudioStreamer* streamer = (AudioStreamer *)CFBridgingRelease(inClientInfo);
...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
		...
		//
		CFHTTPMessageRef message= CFHTTPMessageCreateRequest(NULL, (CFStringRef)@"GET", (CFURLRef)CFBridgingRetain(url), kCFHTTPVersion1_1);
		...
		if (fileLength > 0 && seekByteOffset > 0)
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),
				(CFStringRef)CFBridgingRetain([NSString stringWithFormat:@"bytes=%ld-%ld", seekByteOffset, fileLength]));
			discontinuous = YES;
		}
		...
		if( [[url absoluteString] rangeOfString:@"https"].location != NSNotFound )
		{
			...
			CFReadStreamSetProperty(stream, kCFStreamPropertySSLSettings, CFBridgingRetain(sslSettings));
		}
		...
		//
		CFStreamClientContext context = {0, CFBridgingRetain(self), NULL, NULL, NULL};
...
//			[pool release];
		...
		AudioSessionInitialize (
			...
			CFBridgingRetain(self)                       // data to pass to your interruption listener callback
		);
	...
	eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
			...
			err = AudioFileStreamOpen(CFBridgingRetain(self), MyPropertyListenerProc, MyPacketsProc, 
...
- (void)createQueue
{
	...
	err = AudioQueueNewOutput(&asbd, MyAudioQueueOutputCallback, CFBridgingRetain(self), NULL, NULL, 0, &audioQueue);
	if (err)
	{
	...
	err = AudioQueueAddPropertyListener(audioQueue, kAudioQueueProperty_IsRunning, MyAudioQueueIsRunningCallback, CFBridgingRetain(self));
	if (err)
	{
